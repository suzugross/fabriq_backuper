# ========================================
# Fabriq Extended Hostlist Editor Launcher Build Script (v0.68.0)
# ========================================
# Compiles Launcher_ExtHostlist.cs into Fabriq_ExtHostlist.exe using csc.exe.
# Produces the final binary at the repo root: ..\..\Fabriq_ExtHostlist.exe
# (i.e. E:\fabriq_backuper\Fabriq_ExtHostlist.exe).
#
# Mirrors build_handoffviewer.ps1 exactly except for the source / manifest
...
Show-Separator
Write-Host "  Fabriq Extended Hostlist Editor Launcher Build" -ForegroundColor Cyan
Show-Separator
...
$sourceCs  = Join-Path $scriptDir 'Launcher_ExtHostlist.cs'
$manifest  = Join-Path $scriptDir 'app_exthostlist.manifest'
$outputExe = Join-Path $repoRoot  'Fabriq_ExtHostlist.exe'
...
$running = Get-Process -Name 'Fabriq_ExtHostlist' -ErrorAction SilentlyContinue
if ($running) {
    Show-Warning "Fabriq_ExtHostlist.exe is currently running. Terminate it before rebuild."
    if (-not $Force) {
...
Show-Info "Compiling Launcher_ExtHostlist.cs -> Fabriq_ExtHostlist.exe ..."
