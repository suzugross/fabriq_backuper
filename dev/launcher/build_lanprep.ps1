# ========================================
# Fabriq LAN-Prep Launcher Build Script (v0.24.0)
# ========================================
# Compiles Launcher_LanPrep.cs into Fabriq_LanPrep.exe using csc.exe.
# Produces the final binary at the repo root: ..\..\Fabriq_LanPrep.exe
# (i.e. E:\fabriq_backuper\Fabriq_LanPrep.exe).
#
# Mirrors build_backuper.ps1 exactly except for the source / manifest
# / output file names, so both launchers share the same toolchain.
# ========================================
...
Show-Separator
Write-Host "  Fabriq LAN-Prep Launcher Build" -ForegroundColor Cyan
Show-Separator
...
$sourceCs  = Join-Path $scriptDir 'Launcher_LanPrep.cs'
$manifest  = Join-Path $scriptDir 'app_lanprep.manifest'
$outputExe = Join-Path $repoRoot  'Fabriq_LanPrep.exe'
...
$running = Get-Process -Name 'Fabriq_LanPrep' -ErrorAction SilentlyContinue
if ($running) {
    Show-Warning "Fabriq_LanPrep.exe is currently running. Terminate it before rebuild."
    if (-not $Force) {
...
Show-Info "Compiling Launcher_LanPrep.cs -> Fabriq_LanPrep.exe ..."
