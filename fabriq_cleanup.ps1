# ============================================================
# Fabriq Cleanup - Entry Script (v0.54.0)
# Called from Fabriq_Cleanup.exe (conhost + powershell, asInvoker).
# Standalone post-migration cleanup tool at the same layer as LAN-Prep:
# a dedicated GUI that deletes a hostlist host's leftover backup data
# (backup trees / Desktop handoff folders / LAN-Prep folder).
#
# Split out of the BackUper app (t-0001) so the operator UI stays focused.
# The cleanup ENGINE remains in backuper/common.ps1 (shared with the
# Backuper restore-side delete); this tool dot-sources it directly --
# exactly the way LAN-Prep dot-sources common.ps1 (no vendoring).
#
# Hardening mirrors fabriq_lanprep.ps1: Start-Transcript, top-level trap,
...
$script:CleanupRoot  = Join-Path $PSScriptRoot 'tools\cleanup'
...
$script:TranscriptPath = Join-Path $logDir ("fabriq_cleanup_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
...
# ---- compact passphrase prompt (cleanup-local; verifies via common.ps1) ----
function global:Show-CleanupPassphrasePrompt {
    param([Parameter(Mandatory = $true)][string]$VerifyTokenPath)
    $dlg = New-Object System.Windows.Forms.Form
    Set-FormStyle -Form $dlg -Title 'Fabriq Cleanup - マスターパスフレーズ' -Width 460 -Height 200
    $dlg.KeyPreview = $true
    ...
    $btnOk.Add_Click({
        ...
        if (Test-MasterPassphrase -Passphrase $pp -VerifyTokenPath $VerifyTokenPath) {
            ...
            [System.Windows.Forms.MessageBox]::Show("パスフレーズが一致しません。", "Fabriq Cleanup",
...
# Dot-source backuper libraries (single source of truth) + the cleanup view.
# common (Find-FabriqRoot, Test-MasterPassphrase, Show-*, cleanup engine) ->
# migration_paths -> theme -> fabriq picker -> hostlist reader -> cleanup view.
try {
    ...
    . (Join-Path $script:CleanupRoot 'lib\cleanup_view.ps1')
}
catch {
...
# The cleanup engine (Get-CleanupCandidate / Remove-CleanupArtifact /
# Get-CleanupProtectedRoots / Write-CleanupHistory) reads $script:BackuperRoot.
$script:BackuperRoot = $script:BackuperLib

# Read VERSION (shared with the backuper - both live at the repo root).
$script:CleanupVersion = '0.0.0'
$_verFile = Join-Path $script:RepoRoot 'VERSION'
if (Test-Path $_verFile) { $script:CleanupVersion = (Get-Content -Path $_verFile -Raw).Trim() }

# Optional migration_profile (drives the LAN-Prep candidate scan + protected
# roots). Same load policy as the backuper / LAN-Prep.
...
if (Test-Path -LiteralPath $_profilePath) {
    try {
        ...
        if ($_jsonText -match '"\s*password\s*"') {
            Show-Error "Migration profile contains a 'password' key. Remove it before launching Cleanup."
            return
        }
...
Write-Host ""
Show-Separator
Write-Host "  Fabriq Cleanup  v$($script:CleanupVersion)" -ForegroundColor Cyan
Write-Host "  Post-migration backup-data cleanup (per hostlist host)" -ForegroundColor DarkGray
Show-Separator
Write-Host ""
...
# Without @(), a 1-row hostlist made .Count ambiguous in PS5.1 and the combo
# came up empty even though the row was present (see ps51 list/array quirk).
$script:HostRows = @()
...
if ($_hostsRaw.Count -gt 0) {
    ...
    if ($_needsPp) {
        ...
        $_pp = Show-CleanupPassphrasePrompt -VerifyTokenPath $_verifyToken
...
# ---- build the standalone cleanup window ----
$script:CurrentHost = $null
$form = New-Object System.Windows.Forms.Form
Set-FormStyle -Form $form -Title "Fabriq Cleanup  v$($script:CleanupVersion)" -Width 980 -Height 720
$script:MainForm = $form

$panel = New-CleanupView
$panel.Dock = [System.Windows.Forms.DockStyle]::Fill
$form.Controls.Add($panel)

$form.Add_Shown({ $form.Activate(); Show-CleanupView })
...
[System.Windows.Forms.Application]::Run($form)
    ...
    if ($result -eq 'shown') {
        Write-Host "[ok] Cleanup finished; closing this window." -ForegroundColor Green
    }
    else {
        Read-Host "Press Enter to close this window"
    }
