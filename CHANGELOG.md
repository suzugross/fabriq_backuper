...
- backuper v0.69.3: **リストア開始ボタンの下部余白をバックアップ開始ボタンと統一 (TM t-0013)** —
  リストア開始ボタンが内容領域（フォーム高 810 ≒ 内容 720）をはみ出す Y684（下端 728）で下ギリギリに見えていたのを、バックアップと同じ **Y654**（下端 698・下余白 ≈22px）へ。
  併せてリストア項目グリッドを **H312→H288**（下端 660→636）に縮め、ディスク容量行も **Y666** へ追従。ウィンドウサイズは不変（グリッドは約1行ぶん低くなるがスクロール可）。座標調整のみ・ロジック不変。
...
- backuper v0.69.1: **リストア画面にディスク容量の見積りを小さく表示 (TM t-0013)** —
  ...
  - 選択/参照/更新（t-0008）/対象ユーザ変更で再計算。manifest ラベルは2行容量（H28）を維持し、ディスク行はその2行目位置に**排他オーバーラップ**配置（選択時＝manifest 短1行＋ディスク行／未選択時＝長文2行＋ディスク行クリア）でレイアウト非連鎖。
  - 留意：移行先のステージングは既にディスク上、設定系は handoff（小）なので「空き − 総量」は**やや保守的（安全側）**な目安。多エージェント敵対的レビュー（指摘→修正→再検証 実バグ0）・実機スモーク推奨。
